...
```{r}
df_viz <- clean_data %>% filter(year >= 2018 & year <= 2019)
```
```{r}
ggplot(df_viz, aes(x = country_name, y = pm25_concentration, fill = country_name)) +
  geom_boxplot(outlier.alpha = 0.3) +
  labs(title = "PM25_ Concentration by Continent (2010–2020)",
       y = "PM25_concentration (µg/m³)", x = "") +
  theme_minimal()
```
...
ggplot(data_years, aes(x = country_name, y = pm25_concentration, fill = factor(year))) +
  ...
  labs(
    title = "Comparing PM2.5 Levels by Country (2015 vs 2018)",
    x = "Country",
    y = "PM2.5 (µg/m³)",
    fill = "Year"
  ) +
